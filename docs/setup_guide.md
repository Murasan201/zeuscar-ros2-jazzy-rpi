...
```
[ERROR] [imu_node]: I2Cバス 1 を開けません
[ERROR] [imu_node]: IMU初期化に失敗しました。ノードを終了します。
```

**原因**: ユーザーが `i2c` グループに所属していないため、`/dev/i2c-1` へのアクセス権限がない
...
> **補足**: `sudo i2cdetect -y 1` でIMUが検出されるのにノードが起動しない場合は、この権限問題が原因であることが多い。`i2cdetect` は `sudo` で実行しているため権限エラーにならない。
...
| 2026-02-06 | 10.20 IMUノードI2Cバス権限エラーを追記 |
