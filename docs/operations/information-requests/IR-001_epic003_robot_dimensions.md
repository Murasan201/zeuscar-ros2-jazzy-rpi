...
- **作成日**: 2026-01-12
- **最終更新**: 2026-01-19
- **ステータス**: 情報収集中（本体寸法: 部分完了、LiDAR位置: **完了**）
...
| 項目 | 説明 | 値（mm） | 備考 |
|---|---|---|---|
| X位置 | ロボット中心からの前後オフセット | **+3.5** | 前方が正（実測値）|
| Y位置 | ロボット中心からの左右オフセット | **-4.5** | 左が正、右方向なので負（実測値）|
| Z位置 | 地面からLiDARスキャン面までの高さ | **235** | 実測値 |
| Yaw角度 | 水平面での回転角度 | **+90°** | 前方=0度、反時計回りが正（設計値）|
...
| 項目 | 値 | TF設計への影響 |
|------|----------|----------------|
| X位置（前後） | ロボット中心より**前方に+3.5mm** | X値は**+0.0035m** |
| Y位置（左右） | 中心線から**右方向に-4.5mm** | Y値は**-0.0045m** |
| Z位置（高さ） | 地面から**235mm** | Z値は**+0.235m** |
...
- [x] ~~実測による寸法値の取得（メジャーで測定）~~ → **完了**
  - [x] ~~X位置: ロボット中心からLiDAR中心までの前後距離~~ → **+3.5mm（前方）**
  - [x] ~~Y位置: ロボット中心からLiDAR中心までの左右距離~~ → **-4.5mm（右方向）**
  - [x] ~~Z位置: 地面からLiDARスキャン面までの高さ~~ → **235mm**
...
| 2026-01-19 | LiDAR X位置オフセットを実測値で更新: +3.5mm（前方） |
| 2026-01-19 | LiDAR Y位置オフセットを実測値で更新: -4.5mm（右方向） |
| 2026-01-19 | LiDAR Z位置を実測値で更新: 235mm（地面から）、LiDAR位置計測完了 |
