...
### エージェント分離による TDD 運用（必須）

コーディングが発生する業務では、**テスト作成エージェントと実装エージェントを必ず分離**する。

| フェーズ | 担当エージェント | 成果物 |
|---------|---------------|--------|
| **Red（テスト作成）** | テスト担当エージェント | 失敗するテストコード、テスト設計の意図説明 |
| **Green（実装）** | 実装担当エージェント | テストを通す最小限の実装コード |
| **Refactor** | 実装担当エージェント | リファクタリング後のコード（テスト緑を保持） |

**運用ルール**:
- テスト担当エージェントが先にテストを作成し、完了してから実装担当エージェントが着手する
- テスト担当は仕様・受け入れ基準に基づきテストを設計し、テストの意図を実装担当に伝達する
- 実装担当はテストを変更してはならない（テスト修正が必要な場合はテスト担当にエスカレーション）
- Agent Teams 使用時はタスク依存関係で順序を強制し、チームメイト間の直接通信で設計意図を共有する
- サブエージェント方式の場合は PM がテスト成果物を実装エージェントへのプロンプトに含める
